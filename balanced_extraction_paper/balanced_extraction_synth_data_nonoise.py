...
# We put the simulation data through with no SWOT noise added 
# and include no noise in the covariances so that the extraction is
# giving us the Gaussian smoothed field by G(k)
...
RHO_L_KM         = 16.0                                                         # Gaussian spectral taper scale
COMPUTE_POSTERIOR= True                                                        # toggle posterior on target grid
TAPER_CUTOFF     = 0.0                                                         # "T(k)" cutoff--turned off
OUTNAME          = f"Pass_{pass_num:03d}_Lat{lat_min}_{lat_max}_rho{int(RHO_L_KM)}km"
OUTDIR           = f"./balanced_extraction/SYNTH_data_noiseless/{OUTNAME}/"
...
p_karin, _   = swot.fit_spectrum(karin_NA, spec_ssh_noisy, swot.karin_model) # taper removed now
p_nadir, _   = swot.fit_nadir_spectrum(nadir_NA, spec_nad_noisy, p_karin)
swot.plot_spectral_fits(karin_NA, nadir_NA, p_karin, p_nadir, output_filename = f'{OUTDIR}fits_synth.pdf')
...
rho           = 2 * np.pi * RHO_L_KM                           # ρ for Gaussian smooth--set to 0 above.
...
# covariances used in noisy extraction (not needed here)
...
# Cholesky factor
# -------------------------
eps = 0.0 # 1e-8 * float(np.mean(np.diag(C_obs))) # jiiter option for cholesky if needed
...
with open(f"{OUTDIR}balanced_extraction_pass{pass_num:03d}.pkl", "wb") as f:
...
nc_path = os.path.join(OUTDIR, f"balanced_extraction_pass{pass_num:03d}.nc")
...
if COMPUTE_POSTERIOR:
    ...
    L_obs = la.cholesky(C_obs + eps * np.eye(C_obs.shape[0]), lower=True)      # fix: replaces cho_factor L
